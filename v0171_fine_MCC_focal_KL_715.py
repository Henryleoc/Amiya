...
from torch.nn import Sequential, Linear, ReLU, GELU, Sigmoid, MSELoss, Parameter, HuberLoss, CosineSimilarity, LeakyReLU
from torchmetrics.classification import MulticlassMatthewsCorrCoef, MulticlassRecall, MulticlassPrecision
...
from torch_geometric.utils import remove_self_loops, add_self_loops, mask_feature, mask_to_index, index_to_mask, \
    to_dense_batch, select
...
class EnhancedBipartiteTransform(nn.Module):
    def __init__(self, n_rxn: int, n_met: int, S_init: torch.Tensor, edge_index: torch.Tensor,
        ...
        self.temp_scale = LambdaLayer(lambda x: x / 0.1)

        nn.init.constant_(self.linear3.bias, 0.0)  # 初始化最后一层 bias
    ...
    def forward(self, X_rxn: torch.Tensor):
        ...
        gat_output, gat_weight_pack = self.gat(X_bg, edge_index_flat,
                                               return_attention_weights=True)  # [n_spot * (n_rxn + n_met), n_met]
        ...
        X_met = X_met.unsqueeze(-1)  # [n_spot, n_met, 1]
        B, M, _ = X_met.shape

        # Linear(1 -> 32)
        x = self.linear1(X_met)  # [B, M, 32]
        x = x.view(-1, 32)        # [B*M, 32]
        x = self.bn1(x)
        ...
        X_met_logits = x.view(B, M, 3)  # [n_spot, n_met, 3]

        return (X_met_logits, X_rxn, X_rxn_attn, attn_weight, gat_weight_pack[1])
...
class LitAmiyaFineTune(L.LightningModule):
    def __init__(self, pre_trained_lit_amiya: L.LightningModule, met_edge_index: torch.Tensor,
                 ...
                 num_heads: int = 4, dropout: float = 0.2, S_res_weight: float = 1.0, reg_weight: float = 0.01,
                 lw_corr_mean: float = 0.3, lw_corr: float = 0.5, lw_corr_col: float = 0.5,
                 freeze_layers: bool = True, small_lr_layers: list = None, lr_small: float = 0.0001, lr: float = 0.0005,
                 wd: float = 1e-5,
        ...
        self.amiya.rxn2gene = self._build_new_outer(n_rxn, n_met, S_init, self.rm_edge_index, num_heads, dropout,
                                                    S_res_weight)
    ...
    def _compute_corr(self, X_met: torch.Tensor, X_ref: torch.Tensor, wise: str = 'col', eps: float = 1e-8,
                      X_met_pred=None):
        """
        返回平均的（1 - MCC）或（1 - cosine），作为损失指标
        """
        # 确定维度

        if wise == 'col':
            n_met = X_met.size(1)
            n_class = X_met.size(2)

            mcc_list = []
            for m in range(n_met):
                pred_m = X_met[:, m, :]  # [n_spot, 3]
                pred_label = pred_m.argmax(dim=1)  # [n_spot]

                ref_m_raw = X_ref[:, m]

                ref_m_discrete = ref_m_raw.long()

                if ref_m_discrete.unique().numel() < 2:
                    continue

                mcc_metric = MulticlassMatthewsCorrCoef(num_classes=n_class).to(X_met.device)
                mcc = mcc_metric(pred_label, ref_m_discrete)
                mcc_list.append(mcc)

            if len(mcc_list) == 0:
                print('!!! All metabolites have constant labels (cannot compute MCC)')
                return torch.tensor(0.0, device=X_met.device)

            return 1 - torch.stack(mcc_list).float().mean()

        else:
            # spot-wise结构相似性（cosine similarity）
            pred_label = X_met.argmax(dim=-1).float()  # shape: [n_spot, n_met]
            ref_label = X_ref.float()  # shape: [n_spot, n_met]

            cosine = F.cosine_similarity(pred_label, ref_label, dim=1)  # 每个 spot 一行
    ...
    def _compute_loss(self, X_met: torch.Tensor, X_ref: Tensor, graph_idx: torch.Tensor, prefix: str) -> torch.Tensor:
        ...
        def focal_loss(logits, targets, alpha=None, gamma=1.0):
            ce_loss = F.cross_entropy(logits, targets, reduction='none', weight=alpha)
            pt = torch.exp(-ce_loss)
            loss = ((1 - pt) ** gamma) * ce_loss
            return loss.mean()

        def kl_divergence(p_logit: torch.Tensor, prior: torch.Tensor):
            """
            KL(p || prior)，p 是 softmax 概率分布，prior 是目标先验分布（不需要 log）
            """
            p = F.log_softmax(p_logit, dim=-1)  # [n, 3]
            return F.kl_div(p, prior.expand_as(p), reduction='batchmean')
        ...
        # per-met + focal loss
        loss_ref = 0.0
        for m in range(selected_X_ref.size(1)):
            logits = selected_X_met[:, m, :]
            targets = selected_X_ref[:, m].long()

            # 计算每个代谢物的权重
            class_counts = torch.bincount(targets, minlength=3).float()

            if (class_counts == 0).any():
                # 如果有某一类没有样本 --》 不加权，避免loss为0
                class_weight = None
            else:
            ...
            # focal loss
            loss_m = focal_loss(logits, targets, alpha=class_weight.to(device) if class_weight is not None else None,
                                gamma=2.0)

            with torch.no_grad():
                prior = torch.bincount(targets, minlength = 3).float()
                prior = prior / (prior.sum() + 1e-6)
                prior = prior.to(device)
            loss_kl = kl_divergence(logits, prior)

            loss_m = loss_m + self.lw_kl * loss_kl
            loss_ref += loss_m

        loss_ref = loss_ref / selected_X_ref.size(1)
        ...
        total_loss = (1 - self.lw_corr) * loss_ref + self.lw_corr * loss_corr + self.reg_weight * reg_loss
        ...
        self.log_dict({
            ...
            f'Loss/total/{prefix}': total_loss.item()
    ...
    def _update_metrics(self, pred: torch.Tensor, target: torch.Tensor, prefix: str):
        """
        记录分类精度和结构相似性指标：
        - pred: [n_spot, n_met, 3] 预测 logits
        - target: [n_spot, n_met] 真实标签（整数，0/1/2）
        """

        device = pred.device
        n_class = pred.size(-1)
        n_spot, n_met = target.shape

        # 离散化真实标签
        # NOTE: target here is already a discrete label (0/1/2), DO NOT discretize again!
        # target = discretize_maldi_data(target)

        total_counts = torch.bincount(target.flatten(), minlength=3)
        print(f"标签分布: {total_counts.tolist()}")

        # 获取离散预测结果
        pred_label = pred.argmax(dim=-1)  # [n_spot, n_met]

        # 1. met-wise MCC（每个代谢物一组）
        mcc_metric = MulticlassMatthewsCorrCoef(num_classes=n_class).to(device)
        recall_metric_macro = MulticlassRecall(num_classes=n_class, average='macro').to(device)
        ...
        for m in range(n_met):
            ...
            if target_m.unique().numel() >= 2:
                mcc_value = mcc_metric(pred_m, target_m)
                mcc_list.append(mcc_value)

                recall_value = recall_metric_macro(pred_m, target_m)
                recall_list.append(recall_value)

                # 权重 = 类别越平衡，权重越高
                class_counts = torch.bincount(target_m, minlength=3).float()
                class_imbalance = class_counts.max() / (class_counts.sum() + 1e-6)  # 最大类比例
                weight = 1.0 / (class_imbalance + 1e-6)
                weights.append(weight)

        if len(recall_list) > 0:
            recall_met = torch.stack(recall_list).mean()
        else:
            recall_met = torch.tensor(0.0, device=device)

        if len(mcc_list) > 0:
            weights = torch.tensor(weights, device=pred.device)
            weights = weights / weights.sum()
            mcc_met = torch.sum(torch.stack(mcc_list) * weights)
            # print(f"当前 MCC 权重（前10个代谢物）: {weights[:10].detach().cpu().numpy()}")
        else:
            mcc_met = torch.tensor(0.0, device=pred.device)

        if len(mcc_list) > 0:
            mcc_list_float = [v.item() for v in mcc_list]
            self.mcc_history.append(mcc_list_float)

        # 2. spot-wise cosine similarity（预测标签向量 vs 真实标签向量）
        cos_spot = F.cosine_similarity(pred_label.float(), target.float(), dim=1)  # [n_spot]
        cos_spot_mean = cos_spot.mean()

        recall_metric_per_class = MulticlassRecall(num_classes=n_class, average=None).to(device)
        recall_per_class = recall_metric_per_class(pred_label.flatten(), target.flatten())  # 全部代谢物展开
        print(f"每个类别的 Recall：{recall_per_class.detach().cpu().numpy()}")

        # 每个类别的 Precision
        precision_metric_per_class = MulticlassPrecision(num_classes=n_class, average=None).to(device)
        precision_per_class = precision_metric_per_class(pred_label.flatten(), target.flatten())
        print(f"每个类别的 Precision：{precision_per_class.detach().cpu().numpy()}")

        pred_counts = torch.bincount(pred_label.flatten(), minlength=3)
        print(f"预测类别分布：{pred_counts.tolist()}")

        # 日志记录
        self.log_dict({
            f'MCC_met/{prefix}': mcc_met.item(),
            f'Recall_met/{prefix}': recall_met.item(),
            f'CosSim_spot/{prefix}': cos_spot_mean.item()
        ...
        if self.small_lr_layers:
            ...
            param_groups = [
                ...
                {'params': [p for n, p in self.amiya.named_parameters()
                            if n in self.small_lr_layers], 'lr': self.hparams.lr_small,
                 'weight_decay': self.hparams.wd},
                # Normal LR for the new output layer (and any other trainable params)
                {'params': [p for n, p in self.amiya.named_parameters()
                            if n not in self.small_lr_layers and p.requires_grad], 'lr': self.hparams.lr,
                 'weight_decay': self.hparams.wd}
            ]
        else:
...
def met_id_mapping(query_ids, query_key, target_key,
    ...
    # 从一个代谢物对照表里，查找 ID 对应关系，帮你把一个 ID 转换成另一个 ID。
...
def find_common_items(S1, S2):
    """
    两个 pandas 列表比对，找到相同的元素，返回它们的索引。
...
def discretize_maldi_data(X: torch.Tensor, random_state=0) -> torch.Tensor:
    """
    特殊处理表达为 0 的点，并处理聚类失败的问题：
    - 表达为 0 → 标签 0（单独一类）
    - 其他非零点 → 用 KMeans (n_clusters=2) 聚类，并排序标签（保证 1/2 分别为低/高表达）
    - 如果非零点不够聚类，全部归为 1 类（低表达）

    参数：
        X: [n_spot, n_met] 归一化后的 MALDI 数据

    返回：
        X_discrete: [n_spot, n_met] long 类型，取值为 0 / 1 / 2
    """
    X_discrete = torch.empty_like(X, dtype=torch.long)

    for m in range(X.shape[1]):
        x_col = X[:, m].cpu().numpy()
        labels = np.zeros_like(x_col, dtype=int)  # 先全部置为 0

        non_zero_idx = x_col > 0  # 非零点
        x_nonzero = x_col[non_zero_idx]

        unique_values = np.unique(x_nonzero)

        if len(unique_values) >= 2:
            # 有足够不同的值，正常聚类
            x_nonzero = x_nonzero.reshape(-1, 1)
            kmeans = KMeans(n_clusters=2, random_state=random_state, n_init='auto')
            kmeans_labels = kmeans.fit_predict(x_nonzero)

            # 排序标签
            centers = kmeans.cluster_centers_.flatten()
            sorted_idx = centers.argsort()
            label_map = {old: new + 1 for new, old in enumerate(sorted_idx)}  # 加 1，避免与 0 冲突
            sorted_labels = np.vectorize(label_map.get)(kmeans_labels)

            labels[non_zero_idx] = sorted_labels
        elif len(unique_values) == 1 and len(x_nonzero) > 0:
            # 所有非零点都一样，无法聚类，全部归为 1
            labels[non_zero_idx] = 1  # 全部设为低表达（或者你也可以改成 2）

        X_discrete[:, m] = torch.from_numpy(labels).long()

        # # 打印标签分布
        # unique, counts = np.unique(labels, return_counts=True)
        # print(f"met{m:03d} ->", dict(zip(unique, counts)))
...
def data_preparing(graph_list: str, met_edge: str, S_file: str, avi_genes: str,
                   ...
                   mapID_ref: str = '/data/home/liuzhaoyang/ES/project/MetaSpace/met_model/Amiya_met_meta.tsv',
                   ref_tag: str = 'maldi',
                   verbose: bool = True):
    """
    ...
    if query_metID_key:
        for tmp_i, tmp_g in enumerate(graph_list):
            ...
            if verbose:
                print('graph {}/{}: {} met mapped, {} met found in avi met model'.format(tmp_i + 1, len(graph_list) + 1,
                                                                                         len(mapped_id_dict.keys()),
                                                                                         len(common_index_dict.keys())))
    else:
        mapped_id_dict = {}
        for tmp_i, tmp_g in enumerate(graph_list):
            common_index_dict = find_common_items(
    ...
    for tmp_i in range(len(graph_list)):
        X_maldi_norm = preprocess_maldi_data(graph_list[tmp_i][ref_tag])
        graph_list[tmp_i][ref_tag] = X_maldi_norm

        # 离散化成分类标签
        label_discrete = discretize_maldi_data(X_maldi_norm)
        graph_list[tmp_i][f"{ref_tag}_label"] = label_discrete

    return graph_list, met_edge, S, S_tensor, avi_genes, mapped_id_dict_list, index_mapping_list, rm_edge_index


def setup_training(cfg: FineTuningConfig, pre_trained_lit_amiya: L.LightningModule, graph_list: list,
                   met_edge: torch.Tensor,
                   n_rxn: int, n_met: int, S_init: torch.Tensor, rm_edge_index: torch.Tensor, index_mapping_list: list):
...
def merge_graphs(graph_list: list, ref_tag: str = 'maldi') -> Data:
    ...
    if hasattr(graph_list[0], ref_tag):
        ...
        met_label = torch.cat([g[f"{ref_tag}_label"] for g in graph_list], dim=0)
        return Data(
            x=x, edge_index=edge_index, slide=batch, pos=pos,
            met_ref=met_ref, met_label=met_label, graph_idx=graph_idx)
...
def main():
    ...
    logger = TensorBoardLogger(save_dir=cfg.root_dir, name="Amiya")
    ...
    model, train_data, val_data, test_data = setup_training(cfg, pre_trained_lit_amiya, graph_list, met_edge, n_rxn,
                                                            n_met, S_tensor, rm_edge_index, index_mapping_list)
    ...
    # 保存 mcc_history
    with open("{}/mcc_history.pkl".format(cfg.root_dir), "wb") as f:
        pkl.dump(model.mcc_history, f)

    print(f"[INFO] mcc_history saved to {cfg.root_dir}/mcc_history.pkl")
